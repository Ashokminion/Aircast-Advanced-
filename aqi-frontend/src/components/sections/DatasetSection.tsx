...
const DatasetSection = () => {
  ...
  return (
    <section id="dataset" className="py-24 md:py-32 bg-transparent relative overflow-hidden">
      {/* Background Pattern */}
          ...
          <div className="flex items-center gap-2">
            {Array.from({ length: Math.ceil(features.length / 3) }).map((_, i) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all ${i === carouselIndex ? 'bg-primary w-8' : 'bg-border hover:bg-muted-foreground'
                  }`}
              />
            ))}
          </div>
